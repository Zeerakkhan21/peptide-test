...
// Set as soon as someone engages with the hero form. No longer gates the popup
// — the popup is meant to fire on every landing — but it is still reported, so
// you can see in GTM how often the dialog lands on a half-filled form.
var engaged = false;
...
/* ---------- popup ----------
   Opens CONFIG.popupMs after every landing — first visit, refresh, or a return
   through the Back button. Nothing is stored between page loads, so there is no
   "already seen this" state to reset; reloading the page always re-arms it.
   The only thing that stops it is a lead already submitted in this page load,
   which cannot happen in practice because a submission redirects away. */
var ov = $("#ov"), lastFocus = null, fired = false;

function openPop(trigger){
  if (fired || done || ov.classList.contains("open")) return;
  if (engaged) track("popup_over_form", { note: "opened while the hero form was in progress" });
  fired = true;
...
var popTimer = null;
function armPop(trigger){
  clearTimeout(popTimer);
  popTimer = setTimeout(function(){ openPop(trigger); }, CONFIG.popupMs);
}
armPop("time-" + CONFIG.popupMs + "ms");

/* A Back-button return can restore the page from the browser's cache without
   re-running any of this, which would leave the visitor on the page with no
   popup. `persisted` tells us that happened; re-arm the timer from scratch. */
window.addEventListener("pageshow", function(e){
  if (!e.persisted) return;
  fired = false;
  engaged = false;
  armPop("time-" + CONFIG.popupMs + "ms-restored");
});
